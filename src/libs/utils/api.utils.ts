...
function getBaseURL(env: Environment): string {
  switch (env) {
    case 'local':
      return getLocalURL();
    case 'staging':
      return 'http://192.168.26.21:8080'; // Change when staging server is ready
    case 'production':
    default:
      return 'http://192.168.26.21:8080'; // Change when production server is ready
  }
}
