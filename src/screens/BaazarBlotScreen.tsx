...
const BaazarBlotScreen = ({ navigation, route }: any) => {
    ...
    if (localGameState.currentTurn === 'computer' && localGameState.currentTrick.length === 0) {
      const timer = setTimeout(() => {
        setLocalGameState(prevState => {
          ...
          const stateAfterComputer = baazarBlotAIService.computerMove(prevState);
  ...
  useEffect(() => {
    ...
    if (initialMode === 'ai') {
      ...
      const newGame = baazarBlotAIService.initializeGame();
  ...
  const setupSocketListeners = () => {
    ...
    socketService.onGameEnded((data: any) => {
      ...
      Alert.alert(
        ...
        [{ text: 'OK', onPress: () => navigation.replace('GameMode', {gameType: 'baazar-blot'}) }]
      );
    });
  ...
  const handleFindMatch = async () => {
    ...
    try {
      const matchData = await socketService.findMatch('baazar-blot', userId);
  ...
  const createPrivateRoomOnMount = async () => {
    ...
    try {
      const roomData = await socketService.createPrivateRoom('baazar-blot', userId);
  ...
  const handleLocalPlayCard = (card: Card) => {
    ...
    // Player plays card
    let newState = baazarBlotAIService.playCard(localGameState, card);
    ...
    if (newState.currentTurn === 'computer') {
      setTimeout(() => {
        const stateAfterComputer = baazarBlotAIService.computerMove(newState);
      ...
      }, 600); // Slightly faster delay for Baazar variant
    }
  };
  ...
  const handleLocalGameEnd = async (finalState: LocalGameState) => {
    ...
    // Record game result to backend (using 'baazar-blot' as gameType)
  ...
  const renderMenu = () => (
    <View style={styles.menuContainer}>
      <Text style={styles.title}>Baazar Blot</Text>
      <Text style={styles.subtitle}>Fast-paced variant</Text>
    ...
    return (
      <View style={styles.gameContainer}>
        ...
        <View style={styles.handContainer}>
          <Text style={styles.sectionTitle}>Your Hand ({localGameState.playerHand.length} cards)</Text>
