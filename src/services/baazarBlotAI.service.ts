/**
 * Baazar Blot AI Service - Local single-player game against computer
 * Fast-paced variant of Blot with fewer cards (6 instead of 8)
 */
...
class BaazarBlotAIService {
  /**
   * Initialize a new local Baazar Blot game
   */
  initializeGame(): LocalGameState {
    const deck = this.createDeck();
    const shuffledDeck = this.shuffleDeck(deck);
    
    // Deal 6 cards to each player (faster variant)
    const playerHand = shuffledDeck.slice(0, 6);
    const computerHand = shuffledDeck.slice(6, 12);
...
export const baazarBlotAIService = new BaazarBlotAIService();
